...
class Adxl345:
    ...
    def __init__(self, bus=0, device=0, baud=5000000, mode=3):
        ...
        self.c_buff = []; self.time_s = []; self.time_us = []
    ...
    def start_reading(self, interval=0.1):
        ...
        def read_loop():
            ...
            while not self.stop_thread:
                rst, utime = self.read_fifo()
                time_s = int(utime)
                time_us = round((utime - time_s) * 1000000)
                if rst is not None:
                ...
                if self.Period: # Record data
                    ...
                    with open(f"results/adxl345_data_{t_str}.txt", "w") as f:
                        for i,c in enumerate(self.c_buff):
                            ...
                            f.write("%8d,%d.%06d,%5d,%5d,%5d\n" % (c,self.time_s[i],self.time_us[i],self.x_buff[i],self.y_buff[i],self.z_buff[i]))
                    self.Period = False
