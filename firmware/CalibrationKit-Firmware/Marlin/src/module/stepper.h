...
class Stepper {
  ...
  public:
    ...
    static int32_t triggered_position(const AxisEnum axis);
      ...
      static void do_ulendoStep();

      static bool data_flag[2]; // True means that buffer is empty, so it is ready to be written
      static bool print_flag[2];
      static bool lastBufferDone;
      ...
      typedef struct {
          unsigned char buffer_commands[450];
          int buffer_size;
      } buffer_t;
      static buffer_t motion_buffer[2];
      
    #endif

    #if ENABLED(ULNDO_SINESWEEP_SUPPORT)

      static bool collect_stepperandaccel_data;
      static bool transmit_accelerometer_data;
      
    #endif

    #if ENABLED(ULNDO_SINESWEEP_SUPPORT)   

      static void do_SineStep();

      static float round_(float r);

      static int sign(double x);

      static float sin_gen(float time, float wavelength, float scale_factor);

      static int16_t calib_position;
      ...
      static float move_up_interval;
      static float input_freq;
      static float input_amp;
      static float input_numPeaks;
      static float period;
      static float move_up_period;
      static float normal_period;
