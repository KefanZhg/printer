...
void Stepper::isr() {
  ...
  uint8_t max_loops = 10;
    ...
    #ifdef ULNDO_SOFTWARE_SUPPORT
    static hal_timer_t ticks_elapsed = 0;
    static hal_timer_t next_isr_ticks_prev = 0;
    static uint32_t masterTaskCnt = 0;

    /* Set up fixed time task(s) */

    ticks_elapsed += next_isr_ticks_prev;

    // This section might be expanded to include tasks of different rates
    #define ULNDO_MAX_FREQ 10000
    #define ULNDO_MAX_RATE (STEPPER_TIMER_RATE / ULNDO_MAX_FREQ)
    hal_timer_t ulndo_isr_ticks = ULNDO_MAX_RATE;     // ticks for fastest rate

    // Master
    if(ticks_elapsed >= ULNDO_MAX_RATE){
      ticks_elapsed = 0;
      // Max frequency [10kHz] task
      #ifdef ULNDO_STEPPER_SUPPORT
        do_ulendoStep();
      #endif
      #ifdef ULNDO_SINESWEEP_SUPPORT
        do_SineStep();
      #endif

      // 1000 kHz Task
      if(masterTaskCnt == 9){ // max_freq / task_freq - 1
      ...
      ulndo_isr_ticks = ULNDO_MAX_RATE - ticks_elapsed;   // in the case something else interrupts,
                                                          // only set up the next interrupt time
    }
  #endif
  ...
  do {
    ...
    if (!using_fxtictrl) {
      ...
      TERN_(ULNDO_SOFTWARE_SUPPORT, NOMORE(interval, ulndo_isr_ticks));  // Come back early for Ulendo?
  ...
  #ifdef ULNDO_SOFTWARE_SUPPORT
    next_isr_ticks_prev = next_isr_ticks;
  #endif
...
hal_timer_t Stepper::block_phase_isr() {
  ...
  void Stepper::ulndoAccMeas(){

    uint8_t i = 0;
    if (stepper.collect_stepperandaccel_data || transmit_accelerometer_data){
      
      uint8_t buf[6];

      extDigitalWrite(ULNDO_ACC_SPI_SSEL, LOW);
      DELAY_NS(5); // ADXL min time

      SPI_EXP.transfer(
        #ifdef ULNDO_USING_LIS3DH
          0x28 | 0x80 // Not a bug, MS bit needs to be low with the test device
      ...
      for (uint8_t i = 0; i < 6; ++i)
        buf[i] = SPI_EXP.transfer(0xFF);

      DELAY_NS(5); // ADXL min time
      extDigitalWrite(ULNDO_ACC_SPI_SSEL, HIGH);

      // Tx @ 115200 baud is limited to 10 bytes. More, and the MCU
      // will reset. 120 bits [12 bytes] @ 172800 baud = 0.694 msec
      SERIAL_CHAR(0x01); // Send SOH

      for (uint8_t i = 0; i < 6; ++i)
        SERIAL_CHAR(buf[i]); // Using 48 bits to send
      // only 30 bits of device data - this could be compressed

      // Tx amplitude byte by byte; using SERIAL_ECHO
      // causes intermittent corruption
      #if ENABLED(ULNDO_SINESWEEP_SUPPORT)
        uint8_t amplitude_tx[4];
        memcpy( amplitude_tx, &amplitude, 4);
        for (uint8_t i = 0; i < 4; ++i)
          SERIAL_CHAR(amplitude_tx[i]);
      #endif

      SERIAL_CHAR(0x04); // Send EOT
      SERIAL_FLUSHTX(); // Precautionary
    }
    return;
  }
#endif
...
void Stepper::report_positions() {
...
#if ENABLED(ULNDO_STEPPER_SUPPORT)

  bool Stepper::data_flag[2] = {true, true};
  bool Stepper::print_flag[2] = {false, false};
  bool Stepper::lastBufferDone = true;
  bool Stepper::start_printing = false;
  bool Stepper::print_status = false;

  int Stepper::print_idx = -1;
  int Stepper::block_idx = 0;
  int Stepper::lastBuffer = 0;

  Stepper::buffer_t Stepper::motion_buffer[2] = {0};  

  void Stepper::do_ulendoStep(){ 

    if(lastBufferDone){
      if (print_flag[0]) print_idx = 0;
      else if (print_flag[1] == true) print_idx = 1;
    }

    if (print_idx != -1) {

      lastBufferDone = false;
      ...
      // Read 1 command from current motion buffer; 1 byte stepper command has format:
      // |X_step|Y_step|Z_step|E_step|X_direction|Y_direction|Z_direction|E_direction|
      unsigned char commands = motion_buffer[print_idx].buffer_commands[block_idx];

      DELAY_US(2);
      
      X_DIR_WRITE((commands & 0x40));
      Y_DIR_WRITE(!(commands & 0x10)); // Might need to invert axis if motors Z moves in the opposite direction
      Z_DIR_WRITE((commands & 0x04));
      E0_DIR_WRITE(!(commands & 0x01));

      DELAY_US(2);

      X_STEP_WRITE(commands & 0x80);
      Y_STEP_WRITE(commands & 0x20);
      Z_STEP_WRITE(commands & 0x08);
      E0_STEP_WRITE(commands & 0x02);
      ...
      // Change to the other motion buffer 
      if (block_idx == motion_buffer[print_idx].buffer_size - 1) {
        print_flag[print_idx] = false;
        data_flag[print_idx] = true;

        lastBuffer = print_idx;
  ...
  bool Stepper::collect_stepperandaccel_data = false;
  bool Stepper::transmit_accelerometer_data = false;

#endif

#if ENABLED(ULNDO_SINESWEEP_SUPPORT)

  int16_t Stepper::calib_position = 0;

  float Stepper::data = 0;
  float Stepper::amplitude = 0;
  float Stepper::time = 0;
  float Stepper::frequency = 0;
  float Stepper::omega = 0;
  float Stepper::wavelength = 0;
  float Stepper::move_up_interval = 4;
  float Stepper::input_freq = 0;
  float Stepper::input_amp = 0;
  float Stepper::input_numPeaks = 0;
  float Stepper::period = 0;
  float Stepper::move_up_period = 0;
  float Stepper::normal_period = 0;

  double prev_data = 0,  data = 0;

  float Stepper::round_(float r) {
    return (r>=0.0)?(-(ceil(-r-0.5))):ceil(r-0.5);
  }

  int Stepper::sign(double x) {
    if(x < 0)
      return -1;
    else if(x > 0)
      return 1;
  ...
  float Stepper::sin_gen(float time, float wavelength, float scale_factor) { 
    frequency = 2 / wavelength;
    omega = 2* PI * frequency;
    amplitude = -1 * scale_factor * sin(omega * time);
    
    return amplitude;
  }

  void Stepper::do_SineStep(){

    uint8_t commands = 0;
    
    if (collect_stepperandaccel_data) {
      if(time <= period) {

        ENABLE_AXIS_X();
        ENABLE_AXIS_Y();
        
        wavelength = 2.0 / input_freq;
        if (time <= move_up_period){
          amplitude = sin_gen(time, wavelength, 1.0f*input_amp);
        }
        else if(time > move_up_period && time <= (move_up_period + normal_period)){
          amplitude = sin_gen(time, wavelength, input_amp);
        }
        else if(time > (move_up_period + normal_period)){
          amplitude = sin_gen(time, wavelength, 1.0f*input_amp);
        }

        data = round_(amplitude*planner.settings.axis_steps_per_mm[X_AXIS])/planner.settings.axis_steps_per_mm[X_AXIS];
        int diff = sign(data - prev_data);
        prev_data = data;
    
        // Get the step and direction for each move
        switch (diff) {
          case 0: {commands = 0; break;}
          case 1: {commands = 192; break;}
          case -1: {commands = 128; break;}
        }

        // Collect the (incremental) position of the stepper motor
        if (commands == 128) {
          calib_position--;
        }
        else if (commands == 192) {
          calib_position++;
        }

        DELAY_US(2);
      
        #ifdef INVERT_X_SINE_WAVE
          X_DIR_WRITE(!(commands & 0x40));
        #else
          X_DIR_WRITE((commands & 0x40));
        #endif

        #ifdef INVERT_Y_SINE_WAVE
          Y_DIR_WRITE((commands & 0x40));
        #else
          Y_DIR_WRITE(!(commands & 0x40));
        #endif
      ...
      } /* time not elapsed period */
      else{

        // End the control and data collection when time has elapsed        
        collect_stepperandaccel_data = false;
